...
func _on_TimerSFuel_timeout():
	if not Global.gameover:
		var fuel_new = fuel.instantiate()
		add_child(fuel_new)
		timer_fuel_spawn.wait_time = randf_range(3,15)

func _on_TimerCars_timeout():
	if not Global.gameover:
		var car_new = cars.instantiate()
		add_child(car_new)
		timer_cars.wait_time = randf_range(0.2,0.5)

func _on_TimerTreeLeft_timeout():
	if not Global.gameover:
		var tree_new = trees.instantiate()
		tree_new.transform.origin.x = randf_range(0,44)
		add_child(tree_new)
		timer_tree_left.wait_time = randf_range(0.1,0.9)

func _on_TimerTreeRight_timeout():
	if not Global.gameover:
		var tree_new = trees.instantiate()
		tree_new.transform.origin.x = randf_range(214,254)
		add_child(tree_new)
		timer_tree_right.wait_time = randf_range(0.1,0.9)
...
func _on_TimerFruits_timeout():
	if not Global.gameover:
		var fruit_new = fruits.instantiate()
		add_child(fruit_new)
		timer_fruits.wait_time = randf_range(6,18)
