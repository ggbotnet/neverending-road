...
func _physics_process(_delta):
	...
	if select.value == 1:
		ui_newgame.set("theme_override_colors/font_color",textcolorsel)
		ui_exit.set("theme_override_colors/font_color",textcolor)
		if Input.is_action_just_pressed("keyboard_enter"):
			# warning-ignore:return_value_discarded
			get_tree().change_scene_to_packed(scene_game)
			Global.Reset()
	if select.value == 2:
		ui_newgame.set("theme_override_colors/font_color",textcolor)
		ui_exit.set("theme_override_colors/font_color",textcolorsel)
		if Input.is_action_just_pressed("keyboard_enter"):
			get_tree().quit()
