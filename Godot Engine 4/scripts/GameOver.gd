...
func _physics_process(_delta):
	...
	if select.value == 1:
		ui_contiune.set("theme_override_colors/font_color",textcolorsel)
		ui_end.set("theme_override_colors/font_color",textcolor)
		if Input.is_action_just_pressed("keyboard_enter"):
			# warning-ignore:return_value_discarded
			get_tree().change_scene_to_file("res://scenes/Game.tscn")
			Global.Reset()
	if select.value == 2:
		ui_contiune.set("theme_override_colors/font_color",textcolor)
		ui_end.set("theme_override_colors/font_color",textcolorsel)
		if Input.is_action_just_pressed("keyboard_enter"):
			# warning-ignore:return_value_discarded
